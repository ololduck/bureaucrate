...
# You can also omit `mailboxes_names`, and init() will try to
# get mailbox in `mailboxes_base`
mailboxes = init(mailboxes_base, mailboxes_names)

for message in mailboxes['INBOX']:
    # move the message to mailbox _notifications_, and mark as read if the word
    # 'notification' is in the subject
    ...
    message.is_from('so@gmail.com').move_to('love of my life').star()
